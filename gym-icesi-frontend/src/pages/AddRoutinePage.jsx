...
const AddRoutinePage = () => {
  ...
  useEffect(() => {
    const fetchExercises = async () => {
      try {
        ...
        setError('Error al cargar los ejercicios existentes.');
      }
    };
    fetchExercises();
  }, []);
  ...
  const handleSubmitRoutine = async (e) => {
    ...
    try {
      await customRoutineService.createCustomRoutineWithExercises(routineName, selectedExercises);
          ...
          {selectedExercises.length > 0 ? (
            <div className="mb-4 grid grid-cols-1 md:grid-cols-2 gap-3">
              {selectedExercises.map(ex => (
                <div key={ex.exercise_id} className="flex justify-between items-center bg-gray-100 p-3 rounded-lg">
                  <span>{ex.name}</span>
                  <button
                    type="button"
